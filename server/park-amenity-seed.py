...
from models import db, Amenity, Park, ParkAmenity 
...
with app.app_context():
    ...
    pa_endpoint = f"https://developer.nps.gov/api/v1/amenities/parksplaces?id={acc_query_string}&api_key={api_key}"

    print("Requesting data on accessibility-related amenities at each park...")
    ...
    for element in pa_data['data']:
        ...
        amenity_rec = Amenity.query.filter(Amenity.nps_api_id == amenity['id']).first()
        amenity_id = amenity_rec.id
    ...
    print("Committing amenities and parks to database...")
    db.session.commit()

    print("Park amenities successfully saved to database!")
